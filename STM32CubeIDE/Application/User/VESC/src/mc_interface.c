...
// Sampling variables
#define ADC_SAMPLE_MAX_LEN		1024
...
static volatile PACKET_STATE_t * sample_phandle = 0;
...
static TaskHandle_t sample_send_handle = NULL;
static void sample_send_thread(void * argument);
static TaskHandle_t fault_stop_handle = NULL;
void fault_stop_thread(void * arg);
//static void stat_thread(void * argument);
...
const char* mc_interface_fault_to_string(mc_fault_code fault) {
	...
	case FAULT_CODE_ENCODER_FAULT: return "FAULT_CODE_ENCODER_FAULT"; break;
...
void mc_interface_sample_print_data(debug_sampling_mode mode, uint16_t len, uint8_t decimation, bool raw, PACKET_STATE_t * phandle) {
	...
	if (mode == DEBUG_SAMPLING_SEND_LAST_SAMPLES) {
		xTaskNotifyGive(sample_send_handle);
		//TODO chEvtSignal(sample_send_tp, (eventmask_t) 1);
	} else {
		m_sample_trigger = -1;
		m_sample_now = 0;
		m_sample_len = len;
...
void mc_interface_fault_stop(mc_fault_code fault, bool is_second_motor, bool is_isr) {
	...
	if (is_isr) {
		m_fault_data.from_isr = true;
	} else {
		if(fault_stop_handle) xTaskNotifyGive(fault_stop_handle);
	}
}
...
//				m_phase_samples[m_sample_now] = (uint8_t)(ang / 360.0 * 250.0);
			...
			//TODO m_status_samples[m_sample_now] = mcpwm_get_comm_step() | (mcpwm_read_hall_phase() << 3);
			m_status_samples[m_sample_now] = 0;
...
//	}
	...
	// BMS limits
	//TODO bms_update_limits(&lo_in_min,  &lo_in_max, conf->l_in_current_min, conf->l_in_current_max);
...
//		//TODO g_backup.odometer += odometer - m_motor_1.m_odometer_last;
//		m_motor_1.m_odometer_last = odometer;
//
//		//TODO uint64_t runtime = chVTGetSystemTimeX() / CH_CFG_ST_FREQUENCY;
//
//		// Handle wrap around
...
void sample_send_thread(void * arg){
	...
	for(;;) {
		...
		for (int i = 0;i < len;i++) {
			uint8_t send_buffer[PACKET_SIZE(40)];
			...
			if (m_sample_raw) {
				...
				buffer_append_float32_auto(buffer, ((float)m_ph1_samples[ind_samp] / 4096.0 * V_REG) * ((VPHASE_R1 + VPHASE_R2) / VPHASE_R2) * ADC_VOLTS_PH_FACTOR, &index);
				buffer_append_float32_auto(buffer, ((float)m_ph2_samples[ind_samp] / 4096.0 * V_REG) * ((VPHASE_R1 + VPHASE_R2) / VPHASE_R2) * ADC_VOLTS_PH_FACTOR, &index);
				buffer_append_float32_auto(buffer, ((float)m_ph3_samples[ind_samp] / 4096.0 * V_REG) * ((VPHASE_R1 + VPHASE_R2) / VPHASE_R2) * ADC_VOLTS_PH_FACTOR, &index);
				buffer_append_float32_auto(buffer, ((float)m_vzero_samples[ind_samp] / 4096.0 * V_REG) * ((VPHASE_R1 + VPHASE_R2) / VPHASE_R2) * ADC_VOLTS_INPUT_FACTOR, &index);
			...
			packet_send_packet(send_buffer, index, (PACKET_STATE_t*)sample_phandle);
		}
	}
}
...
void fault_stop_thread(void * arg){
	...
	for(;;) {
		...
		while(ulTaskNotifyTake(pdTRUE, MS_TO_TICKS(5))==0){
			if(m_fault_data.from_isr){
				m_fault_data.from_isr = false;
				break;
			}
		}
		...
		if (mc_interface_dccal_done() && motor->m_fault_now == FAULT_CODE_NONE) {
			...
			utils_sys_lock_cnt();
			volatile int val_samp = 0;//TIM8->CCR1;
