...
void app_adc_start(bool use_rx_tx) {
	...
	stop_now = false;
	xTaskCreate(adc_thread, "APP_ADC", 512, NULL, PRIO_NORMAL, NULL);
}
...
static void app_adc_configure_port(bool use_rx_tx_as_buttons){
	GPIO_InitTypeDef GPIO_InitStruct = {0};

	if (use_rx_tx_as_buttons) {
		GPIO_InitStruct.Pull = GPIO_PULLUP;
		GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
		GPIO_InitStruct.Pin = HW_UART_RX_PIN;

		HAL_GPIO_Init(HW_UART_RX_PORT, &GPIO_InitStruct);

		GPIO_InitStruct.Pull = GPIO_PULLUP;
		GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
		GPIO_InitStruct.Pin = HW_UART_TX_PIN;

		HAL_GPIO_Init(HW_UART_TX_PORT, &GPIO_InitStruct);
	}else{
//TODO:
//		GPIO_InitStruct.Pull = GPIO_PULLUP;
//		GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
...
void adc_thread(void * arg){
	...
	app_adc_configure_port(use_rx_tx_as_buttons);

	is_running = true;

	for(;;) {
		// Sleep for a time according to the specified rate
		systime_t sleep_time = configTICK_RATE_HZ / config.update_rate_hz;
		...
		if (ramp_time > 0.01) {
			const float ramp_step = (float)ST2MS(TimeElapsedSinceX(last_time)) / (ramp_time * 1000.0);
		...
		case ADC_CTRL_TYPE_CURRENT_REV_BUTTON:
			...
			if (fabsf(pwr) < 0.001) {
				ms_without_power += (1000.0 * (float)sleep_time) / (float)configTICK_RATE_HZ;
			}
			break;
