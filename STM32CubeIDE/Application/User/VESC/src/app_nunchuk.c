...
void app_nunchuk_start(void) {
	...
	stop_now = false;
	//hw_start_i2c();
	//xTaskCreate(chuk_thread, "APP_CHUCK", 512, NULL, PRIO_NORMAL, NULL);
}
...
void app_nunchuk_update_output(chuck_data *data) {
	if (!output_running) {
		...
		output_running = true;
		chuck_d.js_y = 128;
		xTaskCreate(output_thread, "APP_OUTP", 512, NULL, PRIO_NORMAL, NULL);
	}
